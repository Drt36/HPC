...
__global__ void crack(char * alphabet, char * numbers, char * encryptedpass){
	...
	char * gencryptpass = (char *)CudaCrypt(genRawPass);
	char * cryptpass = (char *)encryptedpass;
	int  counter= 0;


	//To check Given encypted password with generated encypted password
	for(int i=0;i<=9;i++){

			if(i >= 0 && i < 6){ 
				if(gencryptpass[i] == cryptpass[i]){
					counter++;
				}
			}else if(counter == 9){
			
				printf("%s = Hurray, Given Encrypted password %s is :  %c %c %c %c\n",encryptedpass,CudaCrypt(genRawPass), genRawPass[0],genRawPass[1],genRawPass[2],genRawPass[3]);
			
			}
			
			else{ 
							
				if((int)gencryptpass[i] == (int)cryptpass[i]){
...
int main(int argc, char ** argv){
	...
	//encrypted password
	const char * encryptedpass = "qmpdwy5134";
	...
	cudaMemcpy(gpuPass, encryptedpass, sizeof(char *) * 11, cudaMemcpyHostToDevice);
